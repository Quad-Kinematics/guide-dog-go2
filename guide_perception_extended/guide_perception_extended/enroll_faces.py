...
def parse_args():
    parser = argparse.ArgumentParser(description='Enroll known faces into a face DB pickle for guide_perception_extended.')
    parser.add_argument('--images-dir', required=True, help='Directory containing one subfolder per person.')
    parser.add_argument('--out', required=True, help='Output path for the {name: embedding} pickle file.')
    parser.add_argument('--ctx-id', type=int, default=-1, help='InsightFace ctx_id (-1 = CPU, >=0 = GPU index). Default -1.')
...
def enroll(images_dir, ctx_id):
    ...
    for person in person_dirs:
        ...
        for image_file in image_files:
            ...
            if frame is None:
                print(f'  [warn] {person}/{image_file}: failed to read image, skipping.')
                continue

            faces = face_app.get(frame)
            if not faces:
                print(f'  [warn] {person}/{image_file}: no face detected, skipping.')
                continue

            if len(faces) > 1:
                print(f'  [warn] {person}/{image_file}: multiple faces detected, using largest bbox.')

            face = _largest_face(faces)
            embedding = np.asarray(face.embedding, dtype=np.float64)
            norm = np.linalg.norm(embedding)
            if norm == 0:
                print(f'  [warn] {person}/{image_file}: zero-norm embedding, skipping.')
                continue
            embeddings.append(embedding / norm)

        if not embeddings:
            print(f'  [warn] {person}: zero usable embeddings, skipping person.')
            continue

        mean_embedding = np.mean(embeddings, axis=0)
        mean_norm = np.linalg.norm(mean_embedding)
        if mean_norm == 0:
            print(f'  [warn] {person}: averaged embedding has zero norm, skipping person.')
            continue

        face_db[person] = mean_embedding / mean_norm
        print(f'  [ok] {person}: enrolled using {len(embeddings)}/{len(image_files)} image(s).')
